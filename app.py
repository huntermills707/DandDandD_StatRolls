...
tab1 = dcc.Tab(label='Dice Probabilities', children=[
    ...
    # result plots — now 3 graphs in a 2-column grid
...
def stat_results(n_clicks, dice_data, stat_enable, z, drop_lowest,
    ...
    Returns:
        roll_results (list): combined base + modified roll plots (2 charts)
        stat_results (list): stat total plot (1 chart), or None if disabled
    '''
    # exit if malformed
    if not dice_data:
        return html.Div("No dice to roll!", style={'color': 'red'}), None
    ...
    if not stat_enable:
        # No stat calculations — show only base roll plots (no overlay)
...
# Clientside callback: capture window.innerWidth on load and on every resize.
# The hidden div '_page-load' fires once on mount to seed the store.
...
tab1 = dcc.Tab(label='Dice Probabilities', children=[
    ...
    # dice container — grid columns set dynamically by window-width callback
    html.Div(id='dice-container'),

    # result plots — grid columns set dynamically by window-width callback
    html.Div(id='roll-results'),
    html.Div(id='stat-results'),
])
...
def grid_style(win_width, cols_wide, cols_narrow=1):
    '''
    Return a CSS grid style dict, choosing columns based on window width.

    Parameters:
        win_width  (int): current window width in pixels
        cols_wide  (int): number of columns when window is wide
        cols_narrow (int): number of columns when window is narrow (default 1)

    Returns:
        style (dict): CSS grid style
    '''
    cols = cols_wide if (win_width or 1200) >= NARROW_WIDTH else cols_narrow
    return {
        'display': 'grid',
...
def render_dice(dice_data, win_width):
    style = grid_style(win_width, cols_wide=2)

    if not dice_data:
        return html.Em("No dice in pool. Click 'Add Die' to start."), style

    return [render_die(i, die) for i, die in enumerate(dice_data)], style
...
def stat_results(n_clicks, win_width, dice_data, stat_enable, z, drop_lowest,
                 drop_highest, drop_lowest_stat, drop_highest_stat,
                 replace_lowest_bool, replace_lowest_value,
                 replace_highest_bool, replace_highest_value):
    '''
    ...
    Parameters:
        n_clicks (int): trigger to run function
        win_width (int): current browser window width in pixels
        dice_data (list): die pool
        stat_enable (bool): bool if to run rolls and stats or just stats
        z (int): number of stats
        drop_lowest (int): number of lowest value die to drop from a roll
    ...
    Returns:
        ...
        roll_style (dict): grid style for roll-results div
        stat_children (list|None): stat total plot, or None if disabled
        stat_style (dict): grid style for stat-results div
    '''
    roll_style = grid_style(win_width, cols_wide=2)
    stat_style = grid_style(win_width, cols_wide=1)

    # exit if not yet triggered or malformed
    if not n_clicks or not dice_data:
        return None, roll_style, None, stat_style
    if drop_lowest + drop_highest >= len(dice_data):
        return (html.Div("Dropping too many Dice!", style={'color': 'red'}),
                roll_style, None, stat_style)
    ...
    fig_rp = dcc.Graph('probs', figure=plot(
        ...
        probs2=roll_probs_mod,
        label1='Base Roll',
        label2='Modified Roll (post stat drops/replacements)',
    ))
    ...
    fig_crp = dcc.Graph('cprobs', figure=plot(
        ...
        moments=False,
        probs2=roll_cprobs_mod,
        label1='Base Roll (cumulative)',
        label2='Modified Roll (cumulative)',
    ))
